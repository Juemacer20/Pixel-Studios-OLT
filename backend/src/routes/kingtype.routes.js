...
async function getKingtypeOlt(oltId) {
  const olt = await prisma.oLT.findUnique({ where: { id: oltId } });
  if (!olt) throw Object.assign(new Error('OLT not found'), { status: 404 });
  if ((olt.brand || '').toLowerCase() !== 'kingtype') {
    throw Object.assign(new Error('This OLT is not a KingType device'), { status: 400 });
  }
  return olt;
}
...
router.get('/:oltId/ports', async (req, res, next) => {
  try {
    const olt = await getKingtypeOlt(req.params.oltId);
...
router.post('/:oltId/pon/:ponIndex/onu', checkRole('noc'), async (req, res, next) => {
  try {
    ...
    const result = await adapter.addOnu(parseInt(req.params.ponIndex), { onuId: onuId || 1, profile: profile || 'LINEKTONU', serial, description });
    res.status(201).json({ data: result });
  } catch (e) { next(e); }
});

router.post('/:oltId/pon/:ponIndex/onu/:onuId/activate', checkRole('noc'), async (req, res, next) => {
  try { const olt = await getKingtypeOlt(req.params.oltId); const adapter = getAdapter(olt); const result = await adapter.activateOnu(parseInt(req.params.ponIndex), parseInt(req.params.onuId)); res.json({ data: result }); }
  catch (e) { next(e); }
});

router.post('/:oltId/pon/:ponIndex/onu/:onuId/deactivate', checkRole('noc'), async (req, res, next) => {
  try { const olt = await getKingtypeOlt(req.params.oltId); const adapter = getAdapter(olt); const result = await adapter.deactivateOnu(parseInt(req.params.ponIndex), parseInt(req.params.onuId)); res.json({ data: result }); }
  catch (e) { next(e); }
});

router.post('/:oltId/pon/:ponIndex/onu/:onuId/reboot', checkRole('noc'), async (req, res, next) => {
  try { const olt = await getKingtypeOlt(req.params.oltId); const adapter = getAdapter(olt); const result = await adapter.rebootOnu(parseInt(req.params.ponIndex), parseInt(req.params.onuId)); res.json({ data: result }); }
  catch (e) { next(e); }
});

router.delete('/:oltId/pon/:ponIndex/onu/:onuId', checkRole('admin'), async (req, res, next) => {
  try { const olt = await getKingtypeOlt(req.params.oltId); const adapter = getAdapter(olt); const result = await adapter.deleteOnu(parseInt(req.params.ponIndex), parseInt(req.params.onuId)); res.json({ data: result }); }
  catch (e) { next(e); }
});
...
router.get('/:oltId/profiles', async (req, res, next) => {
  try {
    ...
    const [line, service, alarm, traffic] = await Promise.allSettled([
      adapter.getProfiles('line'), adapter.getProfiles('srv'),
      adapter.getProfiles('alarm'), adapter.getProfiles('traffic'),
    ]);
    res.json({
      data: {
...
router.get('/:oltId/config', async (req, res, next) => {
  try { const olt = await getKingtypeOlt(req.params.oltId); const adapter = getAdapter(olt); const parsed = await adapter.getParsedConfig(); res.json({ data: parsed }); }
  catch (e) { next(e); }
});

router.get('/:oltId/config/text', async (req, res, next) => {
  try { const olt = await getKingtypeOlt(req.params.oltId); const adapter = getAdapter(olt); const text = await adapter.getRunningConfigText(); res.json({ data: { text } }); }
  catch (e) { next(e); }
});

router.post('/:oltId/config/save', checkRole('admin'), async (req, res, next) => {
  try { const olt = await getKingtypeOlt(req.params.oltId); const adapter = getAdapter(olt); const result = await adapter.saveConfig(); res.json({ data: result }); }
  catch (e) { next(e); }
});
...
router.get('/:oltId/autofind', async (req, res, next) => {
  try {
    const olt = await getKingtypeOlt(req.params.oltId);
    const adapter = getAdapter(olt);
    const existing = await prisma.oNT.findMany({ where: { olt_id: req.params.oltId }, select: { serial_number: true } });
    const existingSet = new Set(existing.map(o => o.serial_number));
    const port = req.query.port ? parseInt(req.query.port) : null;
    if (port) {
      const telnetOnus = await adapter.listOnusTelnet(port);
      const unprovisioned = telnetOnus.filter(o => !existingSet.has(o.serialNumber || '')).map(o => ({ ...o, ponPort: port }));
      res.json({ data: { total: telnetOnus.length, unprovisioned, provisioned: telnetOnus.length - unprovisioned.length } });
    } else {
      const results = [];
      for (let p = 1; p <= 8; p++) {
        try {
          const telnetOnus = await adapter.listOnusTelnet(p);
          const unprovisioned = telnetOnus.filter(o => !existingSet.has(o.serialNumber || ''));
          results.push({ port: p, total: telnetOnus.length, provisioned: telnetOnus.length - unprovisioned.length, unprovisioned });
        } catch (e) {
          logger.warn(`KingType autofind port ${p}: ${e.message}`);
          results.push({ port: p, total: 0, provisioned: 0, unprovisioned: [] });
        }
      }
...
router.post('/:oltId/batch/:action', checkRole('noc'), async (req, res, next) => {
  try {
    ...
    for (const onuId of ids) {
      try { const r = await adapter[methodName](pon, parseInt(onuId)); results.push({ onuId, success: r.success, error: r.error }); }
      catch (e) { results.push({ onuId, success: false, error: e.message }); }
    }
