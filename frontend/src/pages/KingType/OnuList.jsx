...
export default function OnuList() {
  ...
  const [newOnu, setNewOnu] = useState({ serial: '', onuId: 1, profile: 'LINEKTONU' });

  const { data: onus, isLoading, refetch } = useQuery({
    queryKey: ['kingtype', id, 'pon', ponIndex, 'onus'],
    queryFn: () => kingtypeAPI.portOnus(id, ponIndex).then(r => r.data?.data ?? r.data),
  ...
  const activateMut = useMutation({
    mutationFn: (onuId) => kingtypeAPI.activateOnu(id, ponIndex, onuId),
    onSuccess: () => { toast.success('Activated'); qc.invalidateQueries({ queryKey: ['kingtype', id, 'pon'] }); },
    onError: () => toast.error('Activate failed'),
  });

  const deactivateMut = useMutation({
    mutationFn: (onuId) => kingtypeAPI.deactivateOnu(id, ponIndex, onuId),
    onSuccess: () => { toast.success('Deactivated'); qc.invalidateQueries({ queryKey: ['kingtype', id, 'pon'] }); },
    onError: () => toast.error('Deactivate failed'),
  });

  const rebootMut = useMutation({
    mutationFn: (onuId) => kingtypeAPI.rebootOnu(id, ponIndex, onuId),
    onSuccess: () => { toast.success('Reboot sent'); },
    onError: () => toast.error('Reboot failed'),
  });

  const deleteMut = useMutation({
    mutationFn: (onuId) => kingtypeAPI.deleteOnu(id, ponIndex, onuId),
    onSuccess: () => { toast.success('ONU deleted'); qc.invalidateQueries({ queryKey: ['kingtype', id, 'pon'] }); },
    onError: () => toast.error('Delete failed'),
  });

  const addMut = useMutation({
    mutationFn: () => kingtypeAPI.addOnu(id, ponIndex, newOnu),
    onSuccess: () => { toast.success('ONU added'); setAdding(false); qc.invalidateQueries({ queryKey: ['kingtype', id, 'pon'] }); },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {adding && (
        <div className="card" style={{ padding: 12, display: 'flex', gap: 10, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div><label style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 2 }}>Serial number</label><input className="input-base" placeholder="KTXXXXXXXX" style={{ width: 200 }} value={newOnu.serial} onChange={e => setNewOnu({...newOnu, serial: e.target.value})} /></div>
          ...
          <div><label style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 2 }}>Profile</label><input className="input-base" placeholder="LINEKTONU" style={{ width: 130 }} value={newOnu.profile} onChange={e => setNewOnu({...newOnu, profile: e.target.value})} /></div>
                ...
                return (
                  <tr key={o.onuId}>
                    <td style={{ fontWeight: 600 }}>
                      <Link to={`/olts/${id}/kingtype/onu/${ponIndex}/${o.onuId}`} style={{ color: 'var(--accent)' }}>{o.onuId}</Link>
                    </td>
